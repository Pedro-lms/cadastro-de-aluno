	...
	public static void main(String[] args) {
		...
		try {
			...
			if (new FuncaoAutentificacao(new Diretor(login, senha))
					.autenticar())/* Trava o sistema pra quem não tiver o contrato */ {
				...
				for (Aluno aluno : maps.get(StatusAluno.APROVADO)) {
					System.out.println(" Resultado = " + aluno.getNome() + aluno.getAlunoAprovado2() + " com média de: "
							+ aluno.getMedia());
				}
			...
			JOptionPane.showInputDialog(null, "Erro de conversão de números" + saida.toString());
			...
			JOptionPane.showInputDialog(null, "Opa, olha o null" + saida.toString());
		} catch (Exception e) { /* Captura todas as exceções */
			e.printStackTrace();
			JOptionPane.showInputDialog(null, "Erro inesperado" + e.getClass().getName());
		} finally { /* Sempre é executado, ocorrendo erros ou não. Por quê? */
			/*
			 * finally sempre é executado, quando precisa executar um processo, acontecendo
			 * erros ou não no sistema
			 */
			JOptionPane.showConfirmDialog(null, "Obrigado por comprar meu software");
		}
 ...
 * aluno1.setCPF(CPF); aluno1.setNomeMae(nomeMae); aluno1.setNomePai(nomePai);
 * aluno1.setDataMatricula(dataMatricula);
 * aluno1.setSerieMatriculado(SerieMatricula); aluno1.setNomeEscola(nomeIES);
 */
 ...
 * JOptionPane.showInputDialog("Qual a data de matricula?"); String
 * SerieMatricula = JOptionPane.showInputDialog("Qual a Série de Matrícula?");
 ...
 * System.out.println(
 * "-----------------------------------------------------------------");
 * aluno2.setNome("Pedro"); aluno2.setIdade(25);
 ...
 * double soma = 0.0;
 * 
 * for(int i; i < n; i++) { soma += [i]; return soma; }
 * 
 * 
 * Aluno aluno = new Aluno(); System.out.println(aluno.toString()); /*Aqui será
 * a descrição do objeto na memória
 */

/*
 * Equals e HashCodes (Diferenciar objetos)
 * 
 * Aluno aluno3 = new Aluno(); aluno3.setNome("Alex");
 * 
